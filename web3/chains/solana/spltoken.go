...
func (t *SPLToken) Chain() basic.Chain {
	return t.chain
}

func (t *SPLToken) TokenInfo() (*basic.TokenInfo, error) {
	return t.TokenInfoChainid(101) // mainnet chainid
}

// TokenInfoChainid
// @param chainId mainnet 101, testnet 102, devnet 103
func (t *SPLToken) TokenInfoChainid(chainId int) (info *basic.TokenInfo, err error) {
	defer basic.CatchPanicAndMapToBasicError(&err)

	fetchApiInfo := func(chainId int) (*basic.TokenInfo, error) {
...
func (t *SPLToken) BalanceOfAddress(address string) (*basic.Balance, error) {
...
func (t *SPLToken) BalanceOfAccount(account basic.Account) (*basic.Balance, error) {
	return t.BalanceOfAddress(account.Address())
}

// BuildTransfer implements basic.Token.

// BuildTransfer
// This method will automatically create an token account for the receiver if receiver does not own it.
func (t *SPLToken) BuildTransfer(sender string, receiver string, amount string) (txn basic.Transaction, err error) {
...
// BuildTransferAll
// This method will automatically create an token account for the receiver if receiver does not own it.
func (t *SPLToken) BuildTransferAll(sender string, receiver string) (txn basic.Transaction, err error) {
...
func (t *SPLToken) BuildTransferAuto(sender, receiver, amount string, transferAll bool, autoCreateAccount bool) (txn basic.Transaction, err error) {
	defer basic.CatchPanicAndMapToBasicError(&err)

	if !IsValidAddress(sender) || !IsValidAddress(receiver) {
		return nil, basic.ErrInvalidAccountAddress
	}
	if transferAll {
		amount = "1"
	}
	...
	if total.Cmp(amountInt) <= 0 {
		return nil, basic.ErrInsufficientBalance
	}
...
func (t *SPLToken) HasCreated(ownerAddress string) (b *basic.OptionalBool, err error) {
	accounts, _, err := t.TokenAccountOfAddress(ownerAddress)
	if err != nil {
		return nil, err
	}
	return basic.NewOptionalBool(len(accounts) > 0), nil
}
