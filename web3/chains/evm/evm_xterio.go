...
func NewXterioHandler(rpc string, privateKey, mnemonic string) (*XterioHandler, error) {
	...
	if privateKey != "" {
		...
		if err != nil {
			log.Error("初始化XterioHandler获取地址失败", "error", err.Error())
			return nil, err
		} else {
		...
		if err != nil {
			log.Error("初始化XterioHandler随机生成钱包账户失败", "error", err.Error())
			return nil, err
		} else {
			p.Address = common.HexToAddress(address)
			p.privateKey = key
	...
	if err != nil {
		log.Error("初始化XterioHandler获取客户端失败", "error", err.Error())
		return nil, err
	}
...
func (h *XterioHandler) ChainId() int64 {
...
func (h *XterioHandler) DefaultGasLimit() uint64 {
	return 21_0000
}

func (h *XterioHandler) ChainName() string {
	return "XTERIO"
}

func (h *XterioHandler) Client() *ethclient.Client {
	return h.client
}

func (h *XterioHandler) RpcClient() *rpc.Client {
	return h.rpcClient
}

// GetEthBalance 获取以太币余额
func (h *XterioHandler) GetEthBalance(address common.Address) (*big.Int, error) {
	return h.Client().BalanceAt(context.Background(), address, nil)
}

// GetTokenBalance 获取代币余额
func (h *XterioHandler) GetTokenBalance(tokenAddress common.Address, ownerAddress common.Address) (*big.Int, error) {
...
func (h *XterioHandler) SendTransaction(toAddress common.Address,
...
// SendEth 发送以太币
func (h *XterioHandler) SendEth(toAddress common.Address, amount *big.Int, gasLimit uint64) (common.Hash, error) {
	return h.SendTransaction(toAddress, amount, gasLimit, nil, nil)
}

// SendToken 发送代币
func (h *XterioHandler) SendToken(tokenAddress common.Address, toAddress common.Address, amount *big.Int) (common.Hash, error) {
	privateKey, err := crypto.HexToECDSA(h.privateKey)
	if err != nil {
		return common.Hash{}, err
	}
...
func (h *XterioHandler) GetTokenDecimals(tokenAddress common.Address) (uint8, error) {
	opts := &bind.CallOpts{}
...
func (h *XterioHandler) CallContract(contractAddress common.Address, abiStr,
	method string, results interface{},
...
func (h *XterioHandler) QueryTransactionStatus(txHash common.Hash) (isPending bool, status bool, err error) {
