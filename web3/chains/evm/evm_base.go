...
func NewBaseHandler(rpc string, privateKey, mnemonic string) (*BaseHandler, error) {
	...
	if privateKey != "" {
		...
		if err != nil {
			log.Error("初始化BaseHandler获取地址失败", "error", err.Error())
			return nil, err
		} else {
		...
		if err != nil {
			log.Error("初始化BaseHandler随机生成钱包账户失败", "error", err.Error())
			return nil, err
		} else {
			p.Address = common.HexToAddress(address)
			p.privateKey = key
	...
	if err != nil {
		log.Error("初始化BaseHandler获取客户端失败", "error", err.Error())
		return nil, err
	}
...
func (h *BaseHandler) ChainId() int64 {
...
func (h *BaseHandler) DefaultGasLimit() uint64 {
	return 21_0000
}

func (h *BaseHandler) ChainName() string {
	return "BASE"
}

func (h *BaseHandler) Client() *ethclient.Client {
	return h.client
}
...
func (h *BaseHandler) GetEthBalance(address common.Address) (*big.Int, error) {
	return h.Client().BalanceAt(context.Background(), address, nil)
}

// GetTokenBalance 获取代币余额
func (h *BaseHandler) GetTokenBalance(tokenAddress common.Address, ownerAddress common.Address) (*big.Int, error) {
...
func (h *BaseHandler) SendTransaction(toAddress common.Address,
...
// SendEth 发送以太币
func (h *BaseHandler) SendEth(toAddress common.Address, amount *big.Int, gasLimit uint64) (common.Hash, error) {
	return h.SendTransaction(toAddress, amount, gasLimit, nil, nil)
}

// SendToken 发送代币
func (h *BaseHandler) SendToken(tokenAddress common.Address,
...
func (h *BaseHandler) GetTokenDecimals(tokenAddress common.Address) (uint8, error) {
	opts := &bind.CallOpts{}
...
func (h *BaseHandler) CallContract(contractAddress common.Address, abiStr,
...
func (h *BaseHandler) QueryTransactionStatus(txHash common.Hash) (isPending bool, status bool, err error) {
