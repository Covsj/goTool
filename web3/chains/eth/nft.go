...
func (a *RSS3NoteAction) Nft() *basic.NFT {
	if a.Tag != TagCollectible {
		return nil
	}
...
func (f *RSS3Fetcher) FetchNFTs(owner string) (map[string][]*basic.NFT, error) {
	f.Limit = 500
	f.NextCursor = ""
	...
	nftGroupd := make(map[string][]*basic.NFT)
...
func (f *RSS3Fetcher) FetchNFTsJsonString(owner string) (*basic.OptionalString, error) {
	...
	return &basic.OptionalString{Value: string(bytes)}, nil
}
